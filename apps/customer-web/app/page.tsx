...
export default function CustomerApp() {
  ...
  async function checkout() {
    ...
    try {
      ...
      setOrder(data);
      setScreen('tracking');
      await refreshTracking(data.masterOrderId);
    } catch (err: any) {
      setError(err.message);
    }
  }

  async function refreshTracking(orderId = order?.masterOrderId) {
    if (!orderId) return;
    try {
      const data = await apiFetch<any>(`/customer/orders/${orderId}/tracking`, { token });
      setTracking(data);
    } catch (err: any) {
      setError(err.message);
    }
  ...
  if (screen === 'login') {
    return (
      <main className="mx-auto min-h-screen max-w-md bg-gray-50 p-4">
        ...
        <input className="mb-3 w-full rounded-2xl border bg-white p-3" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Telefono" />
    ...
    return (
      <main className="mx-auto min-h-screen max-w-md bg-gray-50 p-4 pb-24">
        ...
        <div className="mb-4 flex gap-2 overflow-x-auto">
          <button onClick={() => setCategory('all')} className={`rounded-full px-4 py-2 ${category === 'all' ? 'bg-black text-white' : 'bg-white'}`}>Tutto</button>
          {categories.map((c) => (
            <button key={c} onClick={() => setCategory(c)} className={`rounded-full px-4 py-2 ${category === c ? 'bg-black text-white' : 'bg-white'}`}>{c}</button>
          ))}
        </div>
            ...
            <div key={p.id} className="rounded-2xl bg-white p-4 shadow">
              <div className="flex justify-between gap-3">
                ...
                <div className="text-right">
                  ...
                  <button onClick={() => add(p)} className="mt-2 rounded-full bg-black px-3 py-1 text-white">+</button>
                </div>
              </div>
            </div>
          ))}
        ...
        {cart.map((item) => (
          <div key={item.id} className="mb-3 rounded-2xl bg-white p-4 shadow">
            ...
            <div className="mt-3 flex items-center gap-3">
              <button onClick={() => remove(item.id)} className="rounded-xl border px-3 py-1">-</button>
              <span>{item.quantity}</span>
              <button onClick={() => add(item)} className="rounded-xl border px-3 py-1">+</button>
            </div>
          </div>
        ))}
        <div className="mb-4 rounded-2xl bg-white p-4 shadow">
          <div className="flex justify-between"><span>Subtotale</span><b>{cents(subtotal)}</b></div>
          {subtotal < 2500 && <p className="mt-2 text-sm text-gray-500">Ti mancano {cents(2500 - subtotal)} per la consegna gratuita.</p>}
        </div>
        <button disabled={!cart.length} onClick={loadQuote} className="w-full rounded-2xl bg-black py-4 text-white disabled:opacity-40">Vai al checkout</button>
        {error && <p className="mt-3 rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}
      </main>
    );
  }
    ...
    return (
      <main className="mx-auto min-h-screen max-w-md bg-gray-50 p-4 pb-24">
        ...
        <div className="mb-4 rounded-2xl bg-white p-4 shadow">
          ...
          <div className="flex justify-between"><span>Prodotti</span><span>{cents(quote?.subtotalCents ?? 0)}</span></div>
          <div className="flex justify-between"><span>Delivery</span><span>{cents(quote?.deliveryFeeCents ?? 0)}</span></div>
          <div className="flex justify-between"><span>Sconto cash</span><span>-{cents(quote?.discountCents ?? 0)}</span></div>
          <div className="mt-2 flex justify-between text-lg font-bold"><span>Totale</span><span>{cents(quote?.totalCents ?? 0)}</span></div>
        </div>
        <div className="mb-4 rounded-2xl bg-white p-4 shadow">
          <p className="font-semibold">Pagamento</p>
          <p className="text-sm text-gray-600">Cash alla consegna · demo MVP</p>
        </div>
        <button onClick={checkout} className="w-full rounded-2xl bg-black py-4 text-white">Conferma ordine</button>
        {error && <p className="mt-3 rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto min-h-screen max-w-md bg-gray-50 p-4 pb-24">
      ...
      <div className="mb-4 rounded-2xl bg-white p-4 shadow">
        ...
        <button onClick={() => refreshTracking()} className="mt-3 rounded-xl border px-3 py-2 text-sm">Aggiorna tracking</button>
      </div>
      {tracking?.decisionPayload && (
        <div className="mb-4 rounded-2xl bg-yellow-50 p-4 shadow">
          ...
          <p className="text-sm text-gray-700">Un brand non può completare l’ordine. In MVP gestisci la risoluzione da Admin Console.</p>
          <div className="mt-3 flex gap-2">
            <button onClick={() => customerDecision('continue')} className="rounded-xl border px-3 py-2">Continua partial</button>
            <button onClick={() => customerDecision('cancel')} className="rounded-xl border px-3 py-2">Annulla tutto</button>
          </div>
        </div>
      )}
      <div className="space-y-3">
